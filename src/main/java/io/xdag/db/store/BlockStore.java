...
public class BlockStore {
    ...
    private static final byte[] ORIGINPRETOPDIFF = Hex.decode(       "FFFFFFFFFFFFFFEF");//origin pretop diff
    ...
    public BlockStore(KVSource<byte[], byte[]> index, KVSource<byte[],byte[]> block, KVSource<byte[],byte[]> time,SimpleFileStore simpleFileStore){
    ...
    public synchronized void saveBlock(Block block){
        ...
        if(indexSource.get(BLOCK_SIZE)!=null && BytesUtils.bytesToLong(indexSource.get(BLOCK_SIZE),0,false)!=0){
    ...
    private void saveBlockInfo(Block block){
        indexSource.put(BytesUtils.merge(BLOCK_MAXDIFF,
                block.getHashLow()),BytesUtils.bigIntegerToBytes(block.getDifficulty(),16,false));
        if(block.getMaxDifflink()!=null) {
            indexSource.put(BytesUtils.merge(BLOCK_MAXDIFFLINK,
                    block.getHashLow()), block.getMaxDifflink().getHashLow());
        }
        if(block.getRef() != null) {
            indexSource.put(BytesUtils.merge(BLOCK_REF,
                    block.getHashLow()),block.getRef().getHashLow());
        }
        indexSource.put(BytesUtils.merge(BLOCK_AMOUNT,block.getHashLow()),
                BytesUtils.longToBytes(block.getAmount(),false));
        indexSource.put(BytesUtils.merge(BLOCK_FLAG,block.getHashLow()),
                BytesUtils.intToBytes(block.getFlags(),false));
        indexSource.put(BytesUtils.merge(BLOCK_FEE,block.getHashLow()),
                BytesUtils.longToBytes(block.getFee(),false));
        indexSource.put(BytesUtils.merge(BLOCK_TIME,block.getHashLow()),
                BytesUtils.longToBytes(block.getTimestamp(),false));
        indexSource.put(BytesUtils.merge(BLOCK_HASH,block.getHashLow()),
                block.getHash());
    }
    ...
    public Block getBlockInfoByHash(byte[] hashlow){
        ...
        if(indexSource.get(BytesUtils.merge(BLOCK_MAXDIFF,hashlow))==null) {
            ...
            diff = BytesUtils.bytesToBigInteger(indexSource.get(BytesUtils.merge(BLOCK_MAXDIFF, hashlow)), 0, false);
        }
    ...
    public synchronized void updateBlockKeyIndex(byte[] hashlow,int keyindex){
        indexSource.put(BytesUtils.merge(BLOCK_KEY_INDEX,hashlow),BytesUtils.intToBytes(keyindex,false));
    }

    public synchronized void deleteBlockKeyIndex(byte[] hashlow){
        if(indexSource.get(BytesUtils.merge(BLOCK_KEY_INDEX,hashlow))==null){return;}
    ...
    public int getBlockKeyIndex(byte[] hashlow){
        if(indexSource.get(BytesUtils.merge(BLOCK_KEY_INDEX,hashlow))!=null) {
            return BytesUtils.bytesToInt(indexSource.get(BytesUtils.merge(BLOCK_KEY_INDEX, hashlow)), 0, false);
        }else {
            return -2;//不存在
        }
    }
