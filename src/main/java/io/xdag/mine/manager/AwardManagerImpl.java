...
public class AwardManagerImpl implements AwardManager {
  ...
  /**
   * @param time 时间段
   * @return 错误代码 -1 没有矿工参与挖矿 不进行支付操作 -2 找不到对应的区块hash 或者 结果nonce -3 找不到对应的区块 -4 区块余额不足，不是主块不进行支付 -5
   *     余额分配失败 -6 找不到签名密钥 -7 难度太小 不予支付
   */
  public int payMiners(long time) {
    ...
    if (keyPos < 0) {
      keyPos =
          blockchain.getMemAccount().get(new ByteArrayWrapper(hash)) != null
  ...
  public void doPayments(
    ...
    /**
     * 基金会和转账矿池部分代码 暂时不用 //先支付给基金会 long fundpay = BasicUtils.xdag2amount(payData.fundIncome); byte[]
     * fund = BytesUtils.fixBytes(BasicUtils.address2Hash(Constants.FUND_ADDRESS),8,24);
     *
     * <p>receipt.add(new Address(fund,XDAG_FIELD_OUT,BasicUtils.xdag2amount(payData.fundIncome)));
     * payAmount += payData.fundIncome;
     *
     * <p>//支付给矿池 实际上放着不动 就属于矿池 receipt.add(new
