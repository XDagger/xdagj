...
/**
 * @Classname MinerCalculate
 * @Description 计算一些miner打印参数
 * @Date 2020/5/30 0:20
 * @Created by Myron
 */
public class MinerCalculate {
    ...
    public static void printfMinerStats(Miner miner) {
        ...
        res.append("miner: ").append(address).append("   regTime: ").append(minerRegTime).append("   unpaid: ").append(String.format("%.6f", unpaid)).
                append("   HashRate: ").append(String.format("%.6f", BasicUtils.xdag_log_difficulty2hashrate(miner.getMeanLogDiff()))).append("\n");
        ...
        for (MinerChannel channel: channels.values()) {
            ...
            channelStr.append("ip: ").append(ip).append("   ").append("connTime: ").append(connTime).append("   ").append("unpaid: ").
                    append(String.format("%.6f", unpaidChannel)).append("     ").append("HahRate:  ").append(String.format("%.6f",rate)).append("\n");
    ...
    public static void calculateNopaidShares(MinerChannel channel, byte[] hash, long currentTaskTime) {
    ...
    public static void updateMeanLogDiff(MinerChannel channel, Task task, byte[] hash) {
        ...
        if (channelTime < taskTime) {
            if (channelTime != 0) {
                double meanLogDiff = movingAverageDouble(channel.getMeanLogDiff(),
        ...
        if (minerTaskTime < taskTime) {
            if (minerTaskTime != 0 ) {
                double meanLogDiff = movingAverageDouble(miner.getMeanLogDiff(),
                        BasicUtils.xdag_diff2log(BasicUtils.getDiffByHash(miner.getLastMinHash())),miner.getBoundedTaskCounter());
