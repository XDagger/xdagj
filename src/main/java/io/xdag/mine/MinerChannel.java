...
/**
 * @Classname MinerChannel
 * @Description 矿池和矿工之间连接的channel
...
public class MinerChannel {
    ...
    /** 存放的是连续16个任务本地计算的最大难度
     *  每一轮放的都是最小hash 计算出来的diffs
     * */
    ...
    private MessageFactory createMinerMessageFactory(XdagVersion version) {
        switch (version) {
        case V03:   return new MinerMessageFactory();
    ...
    public void sendBalance() {
        ...
        byte[] data = BytesUtils.merge(BytesUtils.longToBytes(amount,false), BytesUtils.subArray(accountAddressHashLow,8,24));
