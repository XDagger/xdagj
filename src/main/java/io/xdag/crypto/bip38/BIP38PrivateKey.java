...
public class BIP38PrivateKey extends PrefixedChecksummedBytes {
     ...
     */
    public static BIP38PrivateKey fromBase58(Config config, String base58) throws AddressFormatException {
        ...
        return new BIP38PrivateKey(config, bytes, ecMultiply, compressed, hasLotAndSequence, addressHash, content);
    }

    private BIP38PrivateKey(Config config, byte[] bytes, boolean ecMultiply, boolean compressed,
    ...
    public ECKey decrypt(String passphrase) throws BadPassphraseException {
        ...
        Sha256Hash hash = Sha256Hash.twiceOf(LegacyAddress.fromKey(config, key).toString().getBytes(StandardCharsets.US_ASCII));
