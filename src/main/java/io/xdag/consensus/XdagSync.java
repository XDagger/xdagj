  ...
  private void requesetBlocks(long start, long timeInterval) {
    ...
    if (any != null && any.size() != 0) {
      for (XdagChannel channel : any) {
        ...
        ListenableFuture<SumReplyMessage> futureSum =
            channel.getXdag().sendGetsums(start, start + timeInterval);
        if (futureSum != null) {
          Futures.addCallback(
              futureSum,
  ...
  public void processSumsReply(
    ...
    for (int i = 0; i < 16; i++) {
      if ((compareTo(lsums, i * 16, 8, rsums, i * 16, 8) != 0)
        ...
        // 请求request
        ListenableFuture<SumReplyMessage> future =
            channel.getXdag().sendGetsums(starttime + i * (dt), starttime + i * (dt) + dt);
        if (future != null) {
          Futures.addCallback(
              future,
