...
public class Kernel {
	...
	public synchronized void testStart() throws Exception {
		...
		System.out.println("Xdag clinet/server system booting up: network = "
				+ (Config.MainNet == true ? "MainNet" : "TestNet") + ", version " + XdagVersion.V03 + ", user host = ["
				+ config.getNodeIp() + ":" + config.getNodePort() + "]");
...
//        }
		...
		blockStore = new BlockStore(dbFactory.getDB(DatabaseName.INDEX), dbFactory.getDB(DatabaseName.BLOCK),
				dbFactory.getDB(DatabaseName.TIME), dbFactory.getSumsDB());
		blockStore.reset();
