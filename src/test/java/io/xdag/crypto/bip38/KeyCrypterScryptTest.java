...
public class KeyCrypterScryptTest {
    ...
    public void testKeyCrypterGood1() throws KeyCrypterException {
        ...
        log.debug("Original: " + HEX.encode(TEST_BYTES1));
        log.debug("Reborn  : " + HEX.encode(reborn));
        assertEquals(HEX.encode(TEST_BYTES1), HEX.encode(reborn));
    }
    ...
    public void testKeyCrypterGood2() {
        ...
        for (int i = 0; i < numberOfTests; i++) {
            ...
            assertEquals(HEX.encode(plainText.getBytes()), HEX.encode(reconstructedPlainBytes));
        }
    }
    ...
    public void testEncryptDecryptBytes1() throws KeyCrypterException {
        ...
        log.debug("Original: " + HEX.encode(TEST_BYTES1));
        log.debug("Reborn1 : " + HEX.encode(rebornPlainBytes));
        assertEquals(HEX.encode(TEST_BYTES1), HEX.encode(rebornPlainBytes));
    }
    ...
    public void testEncryptDecryptBytes2() throws KeyCrypterException {
        ...
        for (int i = 0; i < 50; i++) {
            ...
            byte[] rebornPlainBytes = keyCrypter.decrypt(data, keyCrypter.deriveKey(PASSWORD1));
            log.debug("Original: (" + i + ") " + HEX.encode(plainBytes));
            log.debug("Reborn1 : (" + i + ") " + HEX.encode(rebornPlainBytes));
            assertEquals(HEX.encode(plainBytes), HEX.encode(rebornPlainBytes));
        }
    }
}