...
public class MnemonicCodeVectorsTest {
    ...
    public void testMnemonicCode() throws Exception {
        final List<String> mnemonicCode = mc.toMnemonic(Hex.decodeHex(vectorEntropy));
        final byte[] seed = MnemonicCode.toSeed(mnemonicCode, vectorPassphrase);
        final byte[] entropy = mc.toEntropy(MnemonicCode.WHITESPACE_SPLITTER.splitToList(vectorMnemonicCode));

        assertEquals(vectorEntropy, Hex.encodeHexString(entropy));
        assertEquals(vectorMnemonicCode, MnemonicCode.SPACE_JOINER.join(mnemonicCode));
        assertEquals(vectorSeed, Hex.encodeHexString(seed));
    }
