...
import static io.xdag.snapshot.config.SnapShotKeys.SNAPTSHOT_KEY_STATE;
import static io.xdag.snapshot.config.SnapShotKeys.SNAPTSHOT_KEY_STATS;
import static io.xdag.snapshot.config.SnapShotKeys.SNAPTSHOT_KEY_TIME;
...
public class LMDBTest {
    ...
    public void testBalance() throws IOException {
        ...
        File file = new File(
                "/Users/punk/Documents/code/java_project/xdagJ_net/snapshot/xdagj/src/test/resources/5000");
...
//        env.close();
    ...
    public void testPubkey() throws IOException {
        ...
        File file = new File(
                "/Users/punk/Documents/code/java_project/xdagJ_net/snapshot/xdagj/src/test/resources/pubkey");
        Env<ByteBuffer> env = create()
                .setMaxReaders(1)
                .setMapSize(268435456)
                .setMaxDbs(4)
        ...
        try (CursorIterable<ByteBuffer> ci = db.iterate(txn, KeyRange.all())) {
            ...
            for (final KeyVal<ByteBuffer> kv : ci) {
                ...
                ECKeyPair ecKeyPair = new ECKeyPair(null,
                        new BigInteger(1, java.util.Arrays.copyOfRange(Bytes.wrapByteBuffer(kv.val()).toArray(), 1,
                ...
                if (i == 0) {
                    ...
                    snapshotStore.savePubKey(Bytes32.wrap(Bytes.wrapByteBuffer(kv.key())).toArray(),
                            ecKeyPair.getPublicKey().toByteArray());
                }
